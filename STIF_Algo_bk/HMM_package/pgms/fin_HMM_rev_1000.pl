...
#				print " KKKK :: $st \t $xx \n";
 ...
 #                               print " REVREV :: $back_st \t $back_xx \n";
				...
				if($ff > -1000 || $back_ff > -1000)
                                {
                                push(@winslid_seq_pos,"*".$string."="."$back_st"."-"."$back_xx"."="."$back_start"."to"."$back_end");
                                $jo = join("",@winslid_seq_pos);

                                $string_rev=reverse $string;

                                push(@winslid_seq_rev_pos,"*"."$string_rev"."="."$st"."-"."$xx"."="."$start"."to"."$end");
...
=cut
				...
				elsif($ff < -1000 || $back_ff < -1000)
				{
                                push(@winslid_seq_pos,"*".$string."="."$back_st"."-"."$back_xx$back_UTR"."="."$back_start"."to"."$back_end");
				$jo = join("",@winslid_seq_pos);

                                $string_rev=reverse $string;
 
                                push(@winslid_seq_rev_pos,"*"."$string_rev"."="."$st"."-"."$xx$UTR"."="."$start"."to"."$end");
				$jo_rev = join("",@winslid_seq_rev_pos); 
=d
                                $string =~tr/aAcCgGtT/tTgGcCaA/;
...
#			print "LLLLLLLLLLLL  :: $winslid_seq[0] ::: $winslid_seq[1]\n";
        ...
        #                       	print "WWWWWWWWWWW :: $filecc[$d]\n";
			...
			scores($winslid_seq_rev[0]);
                        ...
                        if($valid_score[0] != 0)
                        {
                                print "REVERSE\t\t$winslid_seq_rev[0]\t\t$winslid_seq_rev[1]\t\t$winslid_seq_rev[2]\t\t";
